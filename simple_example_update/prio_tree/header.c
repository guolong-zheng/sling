...
BNode * create_tree(int size){
    ...
    for(int i = 0; i < size - 3; i++){
        root = treap_insert_node(root, i, rand_num());
    }
    root = treap_insert_node(root, -5, rand_num());
    root = treap_insert_node(root, -2, rand_num());
    root = treap_insert_node(root, -4, rand_num());
    return root;
}
