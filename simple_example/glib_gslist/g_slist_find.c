...
Node * g_slist_find(Node * list, int data)
  ...
  /*D_ensures ((list^(old(list)) & ((lseg^(old(list), ret) & (~ (data i-in lseg-keys^(old(list), ret)))) * list^(ret))) &
  (((data i-in keys^(old(list))) & ((ret |-> int key: data) * true)) | ((~ (data i-in keys^(old(list)))) & (ret l= nil)))) */
{
	while(list != NULL)
    /*D_invariant (list^(old(list)) & ((lseg^(old(list), list) & (~ (data i-in lseg-keys^(old(list), list)))) * list^(list)))  */
	{
		if (list->key == data) {
			break;
		}
...
int main(){
    ...
    Node * res = g_slist_find(root, (int)rand());
