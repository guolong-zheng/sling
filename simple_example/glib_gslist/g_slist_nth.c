...
Node * g_slist_nth(Node * list, int n)
  /*D_requires (list^(x) & (0 <= n)) */
  /*D_ensures (list^(old(list))
                & (((length^(old(list)) <= old(n)) & (ret l= nil))
                    | ((old(n) < length^(old(list)))
                        & ((lseg^(old(list), ret) & (lseg-length^(old(list), ret) i= old(n))) * list^(ret))))) */
{

	while(n > 0 && list != NULL)
    /*D_invariant ((list^(old(list))
                      & ((lseg^(old(list), list) & ((lseg-length^(old(list), list) + n) i= old(n))) * list^(list)))
                    & (0 <= n)) */
	{
		n--;
		list = list->next;
...
int main(){
    ...
    Node * res = g_slist_nth(root, rand()%5);
    return 0;
}
