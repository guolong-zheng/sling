  ...
  /*D_ensures  sorted-sll^(ret) */
{
  ...
  Node * res = x;
  while(curr != NULL)
    /*D_invariant ((((prv |-> loc next: curr; int key: pk)
                    * (sorted-lseg^(res, prv) & (lseg-keys^(res, prv) set-le pk)))
                   * (sorted-sll^(curr) & (keys^(curr) set-le pk)))
                  | (((prv l= nil) & (res l= curr)) * sorted-sll^(curr))) */
  {
    Node * old_curr = curr;
    curr = curr->next;
    int nondet;
