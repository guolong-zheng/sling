              ...
              * (ret |-> loc next: nill; int key: lastd)) */
{
  SNnode * curr = l;
	if (curr != NULL) {
      ...
      /*D_invariant ( (((sorted-lseg^(y, curr) & (lseg-keys^(y, curr) set-le currd)) 
        * (curr |-> loc next: currn; int key: currd)) 
        * (sorted-sll^(currn) & (currd le-set keys^(currn)))) 
            & (((lseg-keys^(y, curr) union (singleton currd)) union keys^(currn)) s= old(keys^(l))) ) */
		{ 
			curr = curr->next;
		}
	}
