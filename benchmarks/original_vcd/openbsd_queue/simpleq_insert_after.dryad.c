...
void simpleq_insert_after(Node * head, Node * listelm)
  /*D_requires (((nill l= nil) & (head |-> loc next: fst; loc last: lst)) * ((lseg^(fst, listelm) * (((listelm l= lst) & emp) | ((listelm |-> loc next: len) * lseg^(len, lst)))) * (lst |-> loc next: nill))) */
  /*D_ensures  (((nill l= nil) & (head |-> loc next: fst1; loc last: lst1)) * (((lseg^(fst1, listelm) * (listelm |-> loc next: elm)) * (((elm l= lst1) & emp) | ((elm |-> loc next: len) * lseg^(len, lst1)))) * (lst1 |-> loc next: nill))) */
{
